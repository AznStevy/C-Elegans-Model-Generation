...
begin
	dataset_dir = joinpath(workspace_root, dataset)
	json_files = sort(filter(endswith("json"),readdir(dataset_dir)))
end
...
md"Cell: $(@bind cell Select(String.(keys(json_data[1][strain][position][celltype]))))"
...
begin
	...
	Y = x->x[2]
	Z = x->x[3]
	XYZ = data->(X.(data), Y.(data), Z.(data))
	XYZ_stack = data->stack(XYZ(data))
	Plots.plot(XYZ_stack(coordinates1), layout=(3,1), label=["x1" "y1" "z1"], title="$cell", legend=:outerright)
	Plots.plot!(XYZ_stack(coordinates2), layout=(3,1), label=["x2" "y2" "z2"], title="$cell")
	#Plots.plot!(XYZ_stack(coordinates6), layout=(3,1), label=["x6" "y6" "z6"], title="$cell")

	#Plots.xlabel!("Time")
	#Plots.plot(XYZ.(coordinates), label="$cell x")
end
