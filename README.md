...
Make sure that in these filepaths that you have listed, there is either a `CellKey.xlsx` or `cell_key.json` file. If there is a `CellKey.xlsx` file, but no `cell_key.json` file, the program will parse the Excel file and automatically generate the `cell_key.json` file. The program will always prioritize the `.json` file and ignore the `xlsx` file, so make edits in the `.json` file if it's there. This file should look something like what's shown below. If the folder structure of a specific worm is different, reference the [cell key](#cell-key-file) section
